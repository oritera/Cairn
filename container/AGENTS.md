...
# 浏览器
* 容器内 Chromium 运行在虚拟桌面 DISPLAY=:99，通过 http://<HOST_IP>:6080/vnc.html 可远程看到并操作
* 使用 playwright 必须用有头模式
* 遇到验证码、登录等需要人工操作的场景，直接告诉用户打开 noVNC 地址: http://<HOST_IP>:6080/vnc.html
* 不要反复确认，不要长篇描述，直接给出地址和当前状态
