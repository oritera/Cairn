...
# 浏览器
* 容器内有一个有头 Chromium 浏览器运行在虚拟桌面 DISPLAY=:99 上
* 通过 http://<THIS_HOST>:6080/vnc.html 可以远程看到浏览器画面并操作
* 使用 playwright 浏览器时请用有头模式（不要用 headless），这样你可以通过截图看到页面，用户也能通过 VNC 同步看到
* **重要**：遇到需要人工操作的场景（滑动验证码、手动登录、人机验证等），告诉用户打开 noVNC 页面操作，操作完成后通知你继续

# 题目分布
* level 1 的题目偏向 SRC 场景，自动化众测与主流漏洞发现，你需要多做探索。必要时可以使用 playwright 浏览器操作网页（非 headless 模式，用户可通过 noVNC 实时看到并辅助操作）
